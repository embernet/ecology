...
- **Important Note**: This project uses Next.js 15+ (currently 16.3). Dynamic route parameters (`params` and `searchParams`) are asynchronous Promises. You MUST `await` them before accessing their properties (e.g., `const { id } = await params;`). Do not access them synchronously or the app will crash/404.
