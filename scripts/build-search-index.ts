...
function buildIndex() {
  ...
  // Activity entries
  const YEAR_LABELS: Record<string, string> = { y12: 'Year 1/2', y34: 'Year 3/4', y56: 'Year 5/6' };
  ...
  for (const activity of ACTIVITIES) {
    const yearGroups = activity.year_groups.map((y: string) => YEAR_LABELS[y] ?? y).join(' · ');
    const template = TEMPLATE_LABELS[activity.template] ?? activity.template;

    // Gather all searchable text: description, labels, stage names, category names
    const bodyParts: string[] = [
      activity.description,
      ...(activity.label_pool ?? []),
      ...Object.values(activity.name_labels ?? {}),
      ...Object.values(activity.description_labels ?? {}).flat(),
      ...Object.values(activity.stage_labels ?? {}),
      ...(activity.categories ?? []).map((c: { label: string }) => c.label),
      ...(activity.callouts ?? []).map((c: { label: string }) => c.label),
    ].filter(Boolean);

    const body = bodyParts.join(' ');
    const preview = `${activity.description} — ${yearGroups}`;
