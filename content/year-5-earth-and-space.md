---
title: "Year 5: Earth and space"
slug: "year-5-earth-and-space"
---

## Earth and Space

<Requirement id={`q26`} text={`- describe the movement of the Earth, and other planets, relative to the Sun in the solar system
...
<br />
<Guidance id={`g15`} text={`Pupils should be introduced to a model of the Sun and Earth that enables them to explain day and night. Pupils should learn that the Sun is a star at the centre of our solar system and that it has eight planets: Mercury, Venus, Earth, Mars, Jupiter, Saturn, Uranus and Neptune (Pluto was reclassified as a ‘dwarf planet’ in 2006). They should understand that a moon is a celestial body that orbits a planet (Earth has one moon; Jupiter has four large moons and numerous smaller ones).
...
<Ref>*[National Curriculum in England: primary curriculum](https://www.gov.uk/government/publications/national-curriculum-in-england-primary-curriculum)*, Section: "Earth and space", p. 170.</Ref>

## Our Place in Space

<Note id={`t46`} title={`What is the Solar System?`} text={`We live on a planet called Earth, which is part of a giant system of planets, moons, and asteroids that all travel around a star. We call this amazing neighbourhood the **Solar System**!`} />
...
<NatureExample id={`n202`} 
  title={`The Sun`} 
  emoji={`☀️`} 
  facts={`- The Sun is so enormous that you could fit **one million Earths** inside it! If the Sun were a hollow football, you'd need to fill it with one million tiny peas (the Earths).
...
>
<WikiImage filename="The Sun by the Atmospheric Imaging Assembly of NASA's Solar Dynamics Observatory - 20100819.jpg" alt="Image of the sun taken from space by NASA's Solar Dynamics Observatory" />
<WikiImage filename="019 Sunrise in the Serengeti National Park Photo by Giles Laurent.jpg" alt="The sun rising over the Serengeti National Park in Tanzania." />
<WikiImage filename="Magnificent CME Erupts on the Sun - August 31.jpg" alt="A close-up of the sun ejecting a massive amount of solar material into space, taken from space by NASA's Solar Dynamics Observatory in August 2012" />
...
<NatureExample id={`n203`} 
  title={`The Earth`} 
  emoji={`🌎`} 
  facts={`- The Earth is constantly in motion. It takes 365.25 days (one year) to make one full journey, or **orbit**, around the Sun.
...
>
<WikiImage filename="Rotating earth (large).gif" alt="The Earth" />
...
<NatureExample id={`n204`} 
  title={`The Moon`} 
  emoji={`🌖`} 
  facts={`- The Moon doesn't make its own light. It looks bright because it **reflects** the light from the Sun, just like a giant, dusty mirror.
...
>
<WikiImage filename="FullMoon2010.jpg" alt="The Moon, as seen from Earth" />
...
<Note id={`t47`} title={`Why Do We Have Day and Night?`} text={`Day and Night happen because the Earth **rotates** (spins). It's like a spinning top that's also moving around the Sun!`} />




<NatureExample id={`n205`} 
  title={`Daytime`} 
  emoji={`🏙️`} 
  facts={`- The Sun doesn't actually move across the sky! It just looks that way because the Earth is spinning.
...
>
<WikiImage filename="The London Eye, or the Millennium Wheel, River Thames, London (Ank Kumar) 03.jpg" alt="The London Eye during the day" />
...
<NatureExample id={`n206`} 
  title={`Night-time`} 
  emoji={`🌃`} 
  facts={`- It takes the Earth **24 hours** to complete one full rotation, which is why one full day and night cycle lasts 24 hours.
...
>
<WikiImage filename="London at night.jpg" alt="The London Eye during night-time" />
...
<Note id={`t48`} title={`The Planets`} text={`The Earth isn't the only planet orbiting the Sun. There are eight in total, all moving in their own paths (orbits) around our star.`} />




<NatureExample id={`n207`} 
  title={`The Eight Planets`} 
  emoji={`🪐`} 
  facts={`- A fun rhyme (mnemonic) to remember the order is: "My Very Easy Method Just Speeds Up Naming".
...
>
<WikiImage filename="Planets2013.jpg" alt="The eight planets of our Solar System (not to scale)" />
The planets are all spherical bodies that orbit the Sun.

**The order from the Sun is:**<br />
...
<Note id={`t49`} title={`Nature's Navigators and Timekeepers`} text={`The movements of the Earth, Sun, and Moon are not just for us to look at! Many creatures on Earth use these massive objects and their cycles to survive, find food, and navigate the globe.`} />
...
>
<WikiImage filename="Hirundo rustica 147461220.jpg" alt="Swallow in flight" />
<WikiImage filename="Hirundo rustica gutturalis (young s3).jpg" alt="A swallow perched on a fence" />
...
>
<WikiImage filename="Badger-badger.jpg" alt="A badger foraging at night" />
...
>
<WikiImage filename="Limpet shells - Pico.jpg" alt="Limpets on a rock" />
...
>
<WikiImage filename="Baby sea turtles make their way toward the water.jpg" alt="Baby turtles make their way to the sea" />
<WikiImage filename="Terrapene carolina carolina Baby Turtle.jpg" alt="A baby turtle hiding in its shell" />
...
<Activity id={`a83`} 
      title={`Sun-Seekers Plant Race`} 
      description={`Let's see just how much plants need the Sun! We will plant two pots of cress or bean seeds. We'll give them both the same amount of water, but we will place one in a bright, sunny window and the other in a dark cupboard.
...
<Activity id={`a84`} 
      title={`Nocturnal Nature Log`} 
      description={`The Moon and the Earth's rotation (day/night) have a huge effect on animal behaviour.
...
<Activity id={`a85`} 
      title={`Human Orrery`} 
      description={`Let's model the Solar System with our bodies! In a large space (like the hall or playground), choose one person to be the 'Sun' (standing still), one person to be the 'Earth', and one person to be the 'Moon'.
...
- The 'Moon' must walk in a small circle around the spinning, orbiting 'Earth'.
This shows how the Earth, Moon, and Sun all move relative to each other.`} 
    />



<Activity id={`a86`} 
      title={`Day & Night Demo`} 
      description={`We can show why we have day and night in a dark room. You will need a globe (or a plain ball) to be the Earth and a bright torch to be the Sun.
...
<Activity id={`a87`} 
      title={`Make a Shadow Clock (Sundial)`} 
      description={`On a sunny day, go outside to a playground or field.
...
<Reflection id={`r95`} 
      ...
      description={`On a sticky note, draw the Sun, the Earth, and the Moon. Use arrows to show:
...
<Reflection id={`r96`} 
      ...
      description={`With a partner, try to answer this: *If the Sun doesn't actually move across the sky, why does it look like it rises in the morning and sets in the evening?*`} 
    />
...
<Reflection id={`r97`} 
      ...
      description={`In small groups, discuss: *What would happen to day and night if the Earth stopped spinning? What would it be like to live here?*`} 
    />
...
<Reflection id={`r98`} 
      ...
      description={`As a class, discuss: *We learned that scientists' ideas about the Solar System changed from a geocentric (Earth-centred) model to a heliocentric (Sun-centred) model. Why is it important for scientists to be able to change their ideas when they get new evidence?*`} 
    />
